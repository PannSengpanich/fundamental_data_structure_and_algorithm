public class CDLinkedList {
	...
	public void sort() throws Exception { // any sorting will do BUT you must use only 'this' list. No array or any //
											// other data structures allowed!!!
		// selection sort
		DListIterator finding = new DListIterator(header.nextNode);
		DListIterator maxNode = new DListIterator(finding.currentNode);
		DListIterator last = new DListIterator(header.previousNode);

		for (int i = 0; i < size(); i++) {

			while (finding.currentNode != last.currentNode) {
				if (maxNode.currentNode.data < finding.currentNode.nextNode.data) {
					finding.next();
					maxNode.currentNode = finding.currentNode;
				} else
					finding.next();
			}

			if (maxNode.currentNode.data > last.currentNode.data) {
				int temp = last.currentNode.data;
				last.currentNode.data = maxNode.currentNode.data;
				maxNode.currentNode.data = temp;
			}

			finding.currentNode = header.nextNode;
			maxNode.currentNode = finding.currentNode;
			last.previous();
		}
